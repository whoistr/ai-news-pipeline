...
_PROMPT_TEMPLATE = """你是 AI 资讯评分员。请对以下 {count} 条 AI 资讯/论文进行三关评分。

三关评分（每项 1-5 分，3 分为中性）：
1. importance 重要性：是否涉及重大突破/核心模型发布/行业格局变化/广泛影响。5=本领域里程碑级（如旗舰技术报告、重大开源、政策变动），1=无关紧要的细节
2. timeliness 时效性：是否处于 AI 领域 3-6 个月内的热点窗口。5=当下最热门方向，1=已过时或概念陈旧
3. actionability 可操作性：读者能否据此采取行动（如复现/集成/GitHub Star/部署应用/调用 API）。5=开源+有代码+可立即上手，1=纯理论无法落地

资讯类适用标准：新闻(厂商发布/行业动态)+产品(新工具/新模型/新 API)
论文类适用标准：论文(有实验+有数据集)+综述+理论分析

同时为每条生成展示卡片字段（用于日报卡片，必须认真填写）：
    - cn_title: 中文标题(15-25字，含产品名/技术名，英文术语两侧留空格，名词短语要完整别截断)
- rating: 总评 1-5(5=必读，4=强烈推荐，3=值得一看，2=可选，1=了解即可)
    - why_worth: 为什么值得看(说清核心价值，2-3句，口语化，50字内)
    - core_points: 核心要点(3-4条短句，每条10-18字，陈述句)

只输出 JSON 数组，不要 markdown 包裹，不要解释：
[
  {{"idx": 1, "importance": 5, "timeliness": 4, "actionability": 3, "reason": "<=20字",
   "cn_title": "中文标题", "rating": 4, "why_worth": "为什么值得看",
        ...
        try:
            resp = requests.post(
                ...
                json={"model": model, "temperature": 0.3, "max_tokens": 1200 * len(batch),
                      "messages": [{"role": "system", "content": "你是AI资讯评分员，同时负责卡片文案生成(cn_title/why_worth/core_points)。严格按评分标准打分，认真填写所有字段。只输出JSON数组，不要markdown包裹。"},
